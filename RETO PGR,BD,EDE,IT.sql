...
CREATE TABLE PRODUCT (
    ...
    FOREIGN KEY (CODBRAND)
        REFERENCES BRAND (CODBRAND) ON UPDATE CASCADE ON DELETE CASCADE
);
...
CREATE TABLE PURCHASE (
    ...
    FOREIGN KEY (CODPRODUCT)
        REFERENCES PRODUCT (CODPRODUCT) ON UPDATE CASCADE ON DELETE CASCADE,
    FOREIGN KEY (CODUSER)
        REFERENCES USER (CODUSER) ON UPDATE CASCADE ON DELETE CASCADE
);
...
CREATE TABLE CONTAIN (
    ...
    FOREIGN KEY (CODCOMPONENT)
        REFERENCES COMPONENT (CODCOMPONENT) ON UPDATE CASCADE ON DELETE CASCADE
);
...
DELIMITER //
CREATE FUNCTION sellAndSubstractProduct(CODUSER VARCHAR(20), NAMEPROD VARCHAR(50), AMOUNT INT) RETURNS BOOLEAN
READS SQL DATA
BEGIN
    ...
    IF CURRENTSTOCK<STOCKCHECK THEN
		SELECT ("ERROR. The amount of products you want to buy exceeds the product's total stock.");
        SET ERROR=TRUE;
    ELSE
		IF ERROR=FALSE THEN
...
DELIMITER //
CREATE FUNCTION sellAndSubstractComponent(CODUSER VARCHAR(20), NAMEC VARCHAR(50), AMOUNT INT) RETURNS BOOLEAN
READS SQL DATA
BEGIN
	DECLARE CURRENTSTOCK INT DEFAULT 0;
    DECLARE STOCKCHECK INT DEFAULT 0;
	...
	SET CODPROD:=(SELECT CODCOMPONENT FROM COMPONENT WHERE NAMECOMP=NAMEC);
