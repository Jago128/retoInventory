...
DELIMITER //
CREATE FUNCTION sellAndSubstract(CODUSER VARCHAR(20), NAME_I VARCHAR(50), QUANTITY INT, PRICE DOUBLE, PROD BOOLEAN) RETURNS BOOLEAN
READS SQL DATA
BEGIN
    ...
    IF PROD THEN
        SET CODPROD:=(SELECT CODPRODUCT FROM PRODUCT WHERE NAMEP = NAME_I LIMIT 1);
        ...
        END IF;
    ELSE
        SET CODCOMP:=(SELECT CODCOMPONENT FROM COMPONENT WHERE NAMECOMP = NAME_I LIMIT 1);
