	...
	BEGIN
		...
		IF PROD THEN
			...
			ELSE
				...
				INSERT INTO PURCHASE (CODPRODUCT,CODUSER,QUANTITY,TOTALPRICE,DATEP) VALUES
				(CODE, CODUSER, AMOUNT, TOTALPRICE, CURRENTDATE);
			END IF;
		ELSE
...
BEGIN
    ...
    DECLARE C CURSOR FOR SELECT P.NAMEP, P.TYPEP, P.PRICE, P.STOCKPRODUCT FROM PRODUCT P JOIN BRAND B ON P.CODBRAND = B.CODBRAND WHERE B.NAMEBRAND = brandName;
    DECLARE C2 CURSOR FOR SELECT C.NAMECOMP, C.TYPEC, C.PRICECOMP, C.STOCKCOMPONENT FROM COMPONENT C JOIN BRAND B ON C.CODBRAND = B.CODBRAND WHERE B.NAMEBRAND = brandName;
	DECLARE CONTINUE HANDLER FOR NOT FOUND SET END = TRUE;
    ...
    OPEN C;
    FETCH C INTO prodName, prodType, prodPrice, prodStock;
    WHILE NOT fin DO
