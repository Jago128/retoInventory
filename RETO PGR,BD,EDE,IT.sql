...
DELIMITER //
CREATE PROCEDURE sellAndSubstract(CODUSER VARCHAR(20), NAME_I VARCHAR(50), QUANTITY INT, PRICE DOUBLE, PROD BOOLEAN)
	BEGIN
		DECLARE STOCK INT;
		...
		SET STOCK:=(SELECT callSellAndSubstract(NAME_I, PROD));
		IF PROD THEN
			...
			ELSE
				UPDATE PRODUCT SET STOCKPRODUCT = CURRENTSTOCK WHERE CODPRODUCT = COD;
				SET TOTALPRICE = PRICE * QUANTITY;
				SET CURRENTDATE = CURDATE();
				INSERT INTO PURCHASE (CODPRODUCT,CODUSER,QUANTITY,TOTALPRICE,DATEP) VALUES
				(CODPROD, CODUSER, QUANTITY, TOTALPRICE, CURRENTDATE);
			END IF;
		ELSE
			...
			IF STOCKCHECK=STOCK THEN
				SELECT "You cannot buy more products than the amount of stock available.";
			ELSE
				UPDATE COMPONENT SET STOCKCOMPONENT = CURRENTSTOCK WHERE CODCOMPONENT = COD;
				SET CURRENTDATE = CURDATE();
				INSERT INTO BUY (CODCOMPONENT,CODUSER,QUANTITY,TOTALPRICE,DATEB) VALUES
				(COD, CODUSER, QUANTITY, TOTALPRICE, CURRENTDATE);
			END IF;
	END IF;
END//
...
DELIMITER //
CREATE FUNCTION callSellAndSubstract(NAME_I VARCHAR(50), PROD BOOLEAN) RETURNS INT
READS SQL DATA
BEGIN
